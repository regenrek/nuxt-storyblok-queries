...
class StoryblokManagementQueries {
  ...
  async createStory(options = {}, publish = { publish: 1 }) {
    options = { story: options, publish };
    return await this._createData({ options });
  }

  async _createData({ datatype = "stories", options = {} }) {
    return await this.client.post(`spaces/${this.spaceId}/${datatype}/`, options);
  }
  ...
  async assetUpload(file, uploadContainer) {
    ...
    for (let key in uploadContainer.data.fields) {
