  ...
  Object.keys(data).forEach((item) => {
    ...
    if (Array.isArray(value)) {
      result[key] = value.map((item) => {
        if (isObject(item)) item = camelCaseDeep(item)
        return item
      })
      return
    }
    ...
    if (isObject(value)) {
      result[key] = camelCaseDeep(value)
      return
    }
    result[key] = value
  })
...
class StoryblokQueries {
  constructor (ctx) {
    this.ctx = ctx
    this.moduleOptions = <%= JSON.stringify(options) %>
    this.languages = <%= JSON.stringify(options.languages || []) %>
    this.version = '<%= options.version %>'
    this.routesHome = <%= JSON.stringify(options.routesHome) || JSON.stringify(['home']) %>
  ...
  getCurrentStory (options = {}) {
    ...
    let path = route.name === 'index' || this.routesHome.includes(route.name) ? 'home' : route.path
    path = this._getPath(path, options)
  ...
  _loadData ({
    ...
    return this.client
      ...
      .then((res) => {
        ...
        if (this.moduleOptions.formatResponse) {
  ...
  _getPath (path, options) {
    ...
    if (leadingSlashRegex.test(path)) {
      path = path.replace(leadingSlashRegex, '')
    }
  ...
  _getLang (path, options = {}) {
    ...
    const normalizedPath = addTrailingSlash(removeLeadingSlash(path))
